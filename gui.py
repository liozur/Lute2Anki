...
class ImporterGui:
    ...
    def create_widgets(self):
        ...
        self.empty_translation_check_box = QCheckBox(
            "Allow blank translation", self.widget
        )
        ...
        self.adjust_ease_check_box = QCheckBox(
            "Adjust ease based on Lute status", self.widget
        )
        self.include_WKI_check_box = QCheckBox(
            "Include Well known, Ignored", self.widget
        )
    ...
    def create_layout(self):
        ...
        options_layout.addRow(
            self.parents_only_check_box, self.empty_translation_check_box
        )
    ...
    def load_saved_settings(self):
        ...
        if saved_deck in [
            self.deck_options.itemText(i) for i in range(self.deck_options.count())
        ]:
    ...
    def connect_to_lutedb(self):
        ...
        self.db_group.setTitle(
            f"Database Connection - {current_time} ({len(self.terms)} terms)"
        )
        ...
        # Adding infobox in case no terms are loaded
            ...
            log_info("[gui] No terms meet criteria based on chosen filters")
            showInfo("No terms meet criteria based on chosen filters")
    ...
    def create_cards(self):
        ...
        showInfo(
            f"Total {cards_added} cards successfully added to deck {self.selected_deck}\n(ignored {len(self.terms)-cards_added} duplicates)"
        )
